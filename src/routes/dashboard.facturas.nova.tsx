  ...
  head: () => ({
    meta: [
      { title: "Nova factura · Quota Studio" },
      {
        name: "description",
        content:
          "Crie uma factura com número automático, data de vencimento, linhas de artigos e IVA calculado.",
      },
...
function NovaFactura() {
  ...
  const number = "";
  // "" = cliente pontual, sem ficha. Antes vinha aqui um id fixo do tempo do
  // localStorage ("cli-demo-1") que já não existe — o insert rebentava com
  // "invalid input syntax for type uuid".
  const [clientId, setClientId] = useState<string>(cliente ?? "");
  const [client, setClient] = useState({ name: "", nuit: "", email: "", phone: "", address: "" });
  ...
  async function submit(e: React.FormEvent) {
    ...
    const created = await addInvoice({
      ...
      status,
      notes,
      clientId: UUID_RE.test(clientId) ? clientId : undefined,
      client,
      lines: valid,
      payments: [],
    });
  ...
  return (
    <form onSubmit={submit} className="space-y-2">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1.5 text-sm text-muted-foreground">
            Número <span className="font-medium tabular-nums text-foreground">{number}</span>{" "}
      ...
      <div className="divide-y divide-border/60">
        <FormSection
          title="Cliente"
          description="Escolha um cliente da sua base ou preencha manualmente."
        >
          <Field
            label="Cliente"
            htmlFor="cl-pick"
        ...
        <FormSection
          title="Datas e estado"
          description="Vencimento define quando a factura entra em atraso."
        >
          <FieldRow>
        ...
        <FormSection title="Linhas" description="Artigos e serviços facturados, com IVA por linha.">
          <div className="space-y-3">
            {lines.map((l, i) => (
              <div
                key={i}
                className="grid gap-2 rounded-lg border border-border/70 bg-card p-3 sm:grid-cols-[minmax(0,1fr)_70px_110px_80px_36px] sm:items-end"
              >
                ...
                <Field label="IVA %" htmlFor={`ln-v-${i}`}>
                  <input
                    ...
                    className={inputClass}
                    value={l.vat}
                    onChange={(e) => setLine(i, { vat: Number(e.target.value) })}
                  />
                </Field>
                <button
                  type="button"
            ...
            <button
              type="button"
              onClick={() =>
                setLines((prev) => [...prev, { description: "", qty: 1, price: 0, vat: 16 }])
              }
